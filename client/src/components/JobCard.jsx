...
const JobCard = ({ job }) => {
  ...
  return (
    <div className="border p-6 shadow rounded border-gray-200">
      ...
      <p className="text-gray-500 text-sm mt-2" dangerouslySetInnerHTML={{ __html: job.description.slice(0, 150) }}></p>
      <div className="mt-4 flex gap-4 text-sm">
        ...
        <button onClick={() => {navigate(`/apply-job/${job._id}`);window.scrollTo(0, 0)}} className="cursor-pointer text-gray-500 border border-gray-500 rounded px-4 py-2">
          Learn more
        </button>
      </div>
    </div>
